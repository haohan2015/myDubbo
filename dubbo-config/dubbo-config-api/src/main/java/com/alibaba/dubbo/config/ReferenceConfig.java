...
public class ReferenceConfig<T> extends AbstractReferenceConfig {
    ...
    private T createProxy(Map<String, String> map) {
        ...
        if (isJvmRefer) {
            ...
            /**
             * 此处的refprotocol的真实类型是protocol$Adaptive，自适应也不是直接调用inJvmProtocol，首先调用QosProtocolWrapper，接着调用的是ProtocolFilterWrapper
             * ，然后又调用ProtocolListenerWrapper，然后才是真实的调用inJvmProtocol，生成InjvmInvoker，返回到ProtocolListenerWrapper会通知该服务的引用通知，，然后封装为ListenerInvokerWrapper返回
             * 返回到ProtocolFilterWrapper封装引用过滤器，所以此处的Invoker的真实类型是过滤器链条，然后再返回到QosProtocolWrapper，不做任何事情
             */
        ...
        //并做单独存储，是怎么区分哪些类是包装类呢？是通过判断该类是否有所实现接口类型的构造方法，如果有就认为这是一个包装类。
        //这样在通过自适应真实的去获取实现类的时候，首先创建真实的实现类，然后完成IOC的工作，然后判断一下该类有没有包装类
